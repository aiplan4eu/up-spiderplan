...
class TampConstruction02: # (TestCaseProblem):
    ...
    def get_problem(self):
        ...
        occ_map = OccupancyMap(os.path.join(FILE_PATH, "maps", "simple-construction-map.yaml"), (0, 0))
        ...
        material_at = Fluent("material_at", BoolType(), material=t_material, configuration=t_robot_config)
        provider = Fluent("provider", BoolType(), locaction=t_robot_config, material=t_material)
        consumer = Fluent("consumer", BoolType(), locaction=t_robot_config, material=t_material)
        loaded = Fluent("loaded", BoolType(), robot=t_robot, material=t_material)
        connected = Fluent("connected", BoolType(), loc_a=t_robot_config, loc_b=t_robot_config)
        ...
        move = InstantaneousMotionAction("move", robot=t_robot, c_from=t_robot_config, c_to=t_robot_config)
        ...
        load = InstantaneousMotionAction("load", robot=t_robot, loc=t_robot_config, material=t_material)
        ...
        dump = InstantaneousMotionAction("dump", robot=t_robot, loc=t_robot_config, material=t_material)
